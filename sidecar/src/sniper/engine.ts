...
// Context zscore threshold multipliers
// Higher value = more tolerant (effective threshold rises)
const CONTEXT_THRESHOLD_MULTIPLIER: Record<string, number> = {
  deep_work: 1.5,   // dev tools spike during intense work
  build: 2.0,       // node/cargo/rustc legitimately busy
  meeting: 0.7,     // background processes should stay quiet
  gaming: 0.8,      // games need resources for themselves
  media: 0.8,       // media processes need stable resources
  idle: 0.5,        // runaway processes more visible when idle
}

// Build-context process exemptions — never actioned, only logged
const BUILD_EXEMPT_PROCESSES = [
  'node', 'npm', 'npx', 'cargo', 'rustc', 'tsc', 'msbuild',
  'python', 'python3', 'gradle', 'mvn',
]
...
export class SniperEngine extends EventEmitter {
  ...
  // Returns the effective zscore threshold multiplier for the current context
  getContextMultiplier(context: string): number {
    return CONTEXT_THRESHOLD_MULTIPLIER[context] ?? 1.0
  }

  // Returns true if this process should be exempt from sniper actions in the given context
  // Exempt processes are still logged but never actioned
  ...
  ingest(processes: Array<{ name: string; pid: number; cpu_percent: number; memory_mb: number; handle_count: number }>): void {
    for (const proc of processes) {
      ...
      // Apply context multiplier to effective threshold
  ...
  private evaluateWatch(key: string, watch: ProcessWatch): void {
    ...
    // Build-context exemption check — log but skip action
    if (this.shouldExempt(watch.name, this.currentContext)) {
      this.logger.debug(`[exempt: build context] ${watch.name} deviation ${watch.deviation.max_zscore.toFixed(1)}σ — no action taken`)
      this.emitDeferred(watch, `[exempt: build context] ${watch.name} deviation ${watch.deviation.max_zscore.toFixed(1)}σ — no action taken`)
      return
    }

    // Find matching rule (uses effective threshold with context multiplier)
    const rule = this.findRule(watch.name, watch.deviation.max_zscore)
    if (rule === null) return

    // Context exemption check (rule-level exemptions)
  ...
  private findRule(name: string, zscore: number): SniperRule | null {
    const normalized = name.toLowerCase().replace(/\.exe$/i, '')
    // Apply context multiplier to effective threshold for rule matching
    ...
    const specific = this.rules.find(r =>
      ...
      effectiveZscore >= r.zscore_trigger
    )
    if (specific !== undefined) return specific

    const catchAll = this.rules.find(r =>
      r.enabled && r.target_pattern === '' && effectiveZscore >= r.zscore_trigger
    )
